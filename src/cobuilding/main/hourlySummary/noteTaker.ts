...
export function runSummaryAgent(workspace: Workspace): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    session.sendMessage('Use the activity-summary skill to update the daily summary with recent activity. After the summary is written, use the reaction skill to react to the summary with suggestions and relevant resources.');
  });
}
