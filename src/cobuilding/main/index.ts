    ...
    if (FEATURES.MS_WORD_INTEGRATION_ENABLED && FEATURES.MS_WORD_V2_ENABLED) {
      (async () => {
        try {
          ...
          // Start HTTPS proxy for Office add-in (mixed content: HTTPS add-in can't call HTTP)
          try {
            const https = await import('https');
            const http = await import('http');
            const crypto = await import('crypto');
            const certsDir = path.join(app.getAppPath(), 'ms_office_addin', '.certs');
            ...
            if (fs.existsSync(keyPath) && fs.existsSync(certPath)) {
              ...
              try {
                cert = execSync(
                  ...
                  { input: key, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }
                );
              } catch {
                cert = execSync(
            ...
            const httpsServer = https.createServer({ key, cert }, (req, res) => {
              const parsed = new URL(req.url || '/', 'https://localhost:23112');
              const pathname = parsed.pathname === '/' ? '/taskpane.html' : parsed.pathname;

              // Serve static add-in files for known paths
              if (['/taskpane.html', '/commands.html'].includes(pathname) || pathname.startsWith('/icon-')) {
                const filePath = path.join(addinDir, pathname);
              ...
              // Proxy everything else to the HTTP Fastify server
            ...
            httpsServer.listen(23112, '127.0.0.1', () => {
              log.info('[HTTPS Server] Started on https://localhost:23112 (add-in + API proxy)');
            });

            // Auto-trust cert in login keychain
            try {
              const { execSync } = await import('child_process');
              execSync(`security verify-cert -c "${certPath}" 2>&1`, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
            } catch {
              try {
                const os = await import('os');
                const loginKeychain = path.join(os.homedir(), 'Library/Keychains/login.keychain-db');
                const { execSync } = await import('child_process');
                execSync(`security add-trusted-cert -r trustRoot -k "${loginKeychain}" "${certPath}"`, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
                log.info('[HTTPS Server] Certificate trusted in login keychain');
              } catch (certErr) {
                log.warn('[HTTPS Server] Could not auto-trust certificate:', certErr);
              }
            }
          } catch (err) {
            log.error('[HTTPS Proxy] Failed to start:', err);
          }
...
const OFFICE_ADDIN_ID = 'e56ffae3-be6a-463a-a4a2-9ec965f8d2d7';
const OFFICE_ADDIN_MANIFEST_NAME = `${OFFICE_ADDIN_ID}.manifest.xml`;
const OFFICE_WEF_DIRS = {
...
const OFFICE_ADDIN_CERT_PATH = app.isPackaged
  ? path.join(process.resourcesPath, 'ms_office_addin', '.certs', 'localhost.crt')
  : path.join(app.getAppPath(), 'ms_office_addin', '.certs', 'localhost.crt');
const LOGIN_KEYCHAIN = path.join(app.getPath('home'), 'Library/Keychains/login.keychain-db');

function isCertTrusted(): boolean {
  try {
    if (!fs.existsSync(OFFICE_ADDIN_CERT_PATH)) return false;
    require('child_process').execSync(
      `security verify-cert -c "${OFFICE_ADDIN_CERT_PATH}" 2>&1`,
...
ipcMain.handle('officeAddin:status', () => {
  return {
    word: fs.existsSync(path.join(OFFICE_WEF_DIRS.word, OFFICE_ADDIN_MANIFEST_NAME)),
    powerpoint: fs.existsSync(path.join(OFFICE_WEF_DIRS.powerpoint, OFFICE_ADDIN_MANIFEST_NAME)),
    excel: fs.existsSync(path.join(OFFICE_WEF_DIRS.excel, OFFICE_ADDIN_MANIFEST_NAME)),
    certTrusted: isCertTrusted(),
    certExists: fs.existsSync(OFFICE_ADDIN_CERT_PATH),
  };
});

ipcMain.handle('officeAddin:sideload', () => {
  try {
    const manifestSrc = app.isPackaged
      ? path.join(process.resourcesPath, 'ms_office_addin', 'manifest-local.xml')
      : path.join(app.getAppPath(), 'ms_office_addin', 'manifest-local.xml');

    if (!fs.existsSync(manifestSrc)) {
      return { success: false, error: `Manifest not found at ${manifestSrc}` };
    }

    for (const wefDir of Object.values(OFFICE_WEF_DIRS)) {
      fs.mkdirSync(wefDir, { recursive: true });
      fs.copyFileSync(manifestSrc, path.join(wefDir, OFFICE_ADDIN_MANIFEST_NAME));
    }
    return { success: true };
  } catch (err) {
...
ipcMain.handle('officeAddin:remove', () => {
  try {
    for (const wefDir of Object.values(OFFICE_WEF_DIRS)) {
      const manifestPath = path.join(wefDir, OFFICE_ADDIN_MANIFEST_NAME);
...
ipcMain.handle('officeAddin:trustCert', () => {
  try {
    if (!fs.existsSync(OFFICE_ADDIN_CERT_PATH)) {
      const certsDir = path.dirname(OFFICE_ADDIN_CERT_PATH);
      const keyPath = path.join(certsDir, 'localhost.key');
      fs.mkdirSync(certsDir, { recursive: true });
      ...
      fs.writeFileSync(OFFICE_ADDIN_CERT_PATH, certPem);
    }
    require('child_process').execSync(
      `security add-trusted-cert -r trustRoot -k "${LOGIN_KEYCHAIN}" "${OFFICE_ADDIN_CERT_PATH}"`,
      { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] },
    );
    return { success: true };
  } catch (err) {
...
ipcMain.handle('officeAddin:deleteCert', () => {
  try {
    const certsDir = path.dirname(OFFICE_ADDIN_CERT_PATH);
    const keyPath = path.join(certsDir, 'localhost.key');
    if (fs.existsSync(OFFICE_ADDIN_CERT_PATH)) fs.unlinkSync(OFFICE_ADDIN_CERT_PATH);
