...
export function useNotesHistoryAdapter(dayFile: string): ThreadHistoryAdapter {
  const sessionId = `notes-assistant-${dayFile}`;

  return useMemo(
    (): ThreadHistoryAdapter => ({
      async load() {
        const dbMessages = await window.sessionsAPI.listMessages(sessionId);
...
function convertAssistantContent(content: string, toolResults: ToolResultsMap) {
  ...
  // Haiku notes assistant format: {text: "response"}
  ...
  // Agent SDK format: [{type: 'text', text: '...'}, {type: 'tool_use', ...}]
