...
function useWordPollWebSocket(
  ...
  token: string | null,
  apiBaseUrl: string
): { badgeCount: number; isReviewing: boolean; reviewStartedAt: number | null; shouldShowButtonV2: boolean; focusedWid: string | null } {
  ...
  return { badgeCount, isReviewing, reviewStartedAt, shouldShowButtonV2, focusedWid };
}
...
const AcademiaNotificationsButtonV2: React.FC = () => {
  ...
  const { badgeCount, isReviewing, shouldShowButtonV2, focusedWid } = useWordPollWebSocket(
    widParam,
    tokenParam,
    serverUrl
  );
