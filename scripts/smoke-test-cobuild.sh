...
echo "Running cobuild smoke test with network sandbox..."

LOG_FILE=$(mktemp /tmp/smoke-test-cobuild-XXXXXX)
...
while kill -0 "$SMOKE_PID" 2>/dev/null; do
  if [ "$SECONDS" -ge 60 ]; then
    echo "ERROR: Cobuild smoke test timed out after 60 seconds"
    echo "--- App output ---"
    cat "$LOG_FILE"
    echo "--- End output ---"
...
if [ "$EXIT_CODE" -ne 0 ]; then
  echo "ERROR: Cobuild smoke test failed with exit code $EXIT_CODE"
  echo "--- App output ---"
  cat "$LOG_FILE"
  echo "--- End output ---"
  rm -f "$LOG_FILE"
...
rm -f "$LOG_FILE"
echo "Cobuild smoke test passed"
